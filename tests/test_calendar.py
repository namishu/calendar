...
def test_full_year_pdf(tmp_path: Path) -> None:
    ...
    for page, month in zip(reader.pages, CalendarApp().config["header"]["months"], strict=True):
...
def test_month_grid(year: int, month: int, first_day: int, rows: int) -> None:
    ...
    instance = MonthCalendarInstance(year, month, cfg["header"]["months"], cfg["weekday"]["names"], first_day)
    ...
    assert instance.weekday_texts()[0] == cfg["weekday"]["names"][first_day]
...
def test_partial_config_keeps_bundled_font(tmp_path: Path) -> None:
    ...
    assert Path(app.config["font"]["path"]) == DATA_DIR / "fonts/NotoSansSC-Light.ttf"
...
def test_relative_custom_font(tmp_path: Path, monkeypatch) -> None:
    ...
    directory.mkdir()
    (directory / "font.ttf").symlink_to(DATA_DIR / "fonts/NotoSansSC-Light.ttf")
...
def test_config_is_a_snapshot(tmp_path: Path) -> None:
    ...
    app = CalendarApp(config)
    config.write_text("weekday:\n  first_day: 0\n")
    assert app.config["weekday"]["first_day"] == 6
    assert CalendarApp(config).config["weekday"]["first_day"] == 0
